...
public class BoardMgr {
	...
	public void insertBoard(HttpServletRequest req) {
		...
		try {
			...
			sql = "insert tblBoard(name,content,subject,ref,pos,depth,regdate,count,ip,filename,filesize,secret)";
			...
			pstmt.setString(8, multi.getParameter("secret"));
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public void replyBoard(BoardBean bean) {
		...
		try {
			con = pool.getConnection();
			sql = "insert tblBoard (name,content,subject,ref,pos,depth,regdate,count,ip,secret)";
			sql += "values(?,?,?,?,?,?,now(),?,0,?)";
	...
	public void post1000(){
		...
		try {
			con = pool.getConnection();
			sql = "insert tblBoard(name,content,subject,ref,pos,depth,regdate,pass,count,ip,filename,filesize,secret)";
	...
	public String getSecret(int num) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		...
		try {
			con = pool.getConnection();
			sql = "select secret from tblBoard where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
			if (rs.next()) {
	...
	public void deleteData(int num) {
			...
			try {
				...
				sql = "update tblBoard set name=?, subject=?,content=? where num=?";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "[삭제된글]");
				pstmt.setString(2, "[remove]");
				pstmt.setString(3, "[remove]");
				pstmt.setInt(4, num);
				pstmt.executeUpdate();
			} catch (Exception e) {
				e.printStackTrace();
