...
public class MemberMgr {
	...
	public boolean insertMember(MemberBean bean) {
		...
		try {
			...
			sql = "insert tblMember(id,pwd,name,gender,birthday,email,zipcode"
					+ ",address,hobby,job,national,jobcode)values(?,?,?,?,?,?,?,?,?,?,?,?)";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, bean.getId());
			String rawPassword = bean.getPwd();
			String pwd = SHA256.encodeSha256(rawPassword);
			pstmt.setString(2, pwd); // 암호화된 비밀번호를 쿼리에 적용
			...
			pstmt.setString(10, getJobname(bean.getJobcode()));
			pstmt.setString(11, bean.getNational());
			pstmt.setString(12, bean.getJobcode());
	...
	public boolean loginMember(String id, String pwd) {
		...
		try {
			...
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, id);
			String password = SHA256.encodeSha256(pwd);
	...
	public MemberBean getMember(String id) {
		...
		try {
			...
			if (rs.next()) {
				...
				bean.setNational(rs.getString("national"));
	...
	public boolean updateMember(MemberBean bean) {
		...
		try {
			...
			String sql = "update tblMember set pwd=?, name=?, gender=?, birthday=?,"
					+ "email=?, zipcode=?, address=?, hobby=?, national=?, jobcode=? where id = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, bean.getId());
	...
	public String getJobname(String jobcode) {
		...
		try {
			...
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, jobcode); // 바인딩 변수에 값을 설정합니다.
			rs = pstmt.executeQuery();
			if (rs.next()) {
